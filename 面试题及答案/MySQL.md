...
如果碰到这种情况，首先可以考虑，就是使用 force index，强制走一个索引，这个是不太好的，就是作为一个业务的应急预案，因为它可能牵扯到别的数据库里面就不支持了，他还需要我们就是做一个代码的重新发布，可以考虑使用索引覆盖的最左匹配原则，这种原则的考虑，能不能把这个选错的索引给删掉，这样也是一个优化的方案
